...
let handler = async (m, { conn, args: [effect], text: txt, usedPrefix, command }) => {
  ...
  if (!effects.find(v => (new RegExp(v.title, 'gi')).test(effect))) throw `⏩`
  let text = txt.replace(new RegExp(effect, 'gi'), '').trimStart()
  if (text.includes(split)) text = text.split(split)
  text = Array.isArray(text) ? text : [text]
  let res = await textpro(effect, ...text)
  if (typeof res == 'number') throw res == -1 ? `Efek *${effect}* සොයා ගත නොහැක` : `Gunakan format ${usedPrefix}${command} ${effect} ${new Array(res).fill('text').map((v, i) => v + (i ? i + 1 : '')).join('|')}`
